...
output_dir_train_male = data_dir+'gender/train/male'
output_dir_train_female = data_dir+'gender/train/female'
...
output_dir_test_male = data_dir+'gender/test/male'
output_dir_test_female = data_dir+'gender/test/female'
...
for image in D_train.values:
    ...
    if image[0] == 'male':
        cv2.imwrite(data_dir+'gender/train/male/' + str(counter) + '.jpg', img)
    else:
        cv2.imwrite(data_dir+'gender/train/female/' + str(counter) + '.jpg', img)
...
for image in D_test.values:
    ...
    img = cv2.resize(img, (64,64))
    if image[0] == 'male':
        cv2.imwrite(data_dir+'gender/test/male/' + str(counter) + '.jpg', img)
    else:
        cv2.imwrite(data_dir+'gender/test/female/' + str(counter) + '.jpg', img)
    print('--('+str(counter)+')Processing--')
    counter += 1
