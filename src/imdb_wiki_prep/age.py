...
for i in range(102):
    output_dir_train_male = train_dir + str(i)
    output_dir_train_female = train_dir + str(i)
    ...
    output_dir_test_male = test_dir + str(i)
    output_dir_test_female = test_dir + str(i)
...
for image in D_train:
    ...
    cv2.imwrite(train_dir + str(image[0]) + '/' + str(counter) + '.jpg', img)
...
for image in D_test:
    img = cv2.imread(image[1], 1)
    img = cv2.resize(img, (128,128))
    cv2.imwrite(test_dir + str(image[0]) +  '/' + str(counter) + '.jpg', img)
    print('--('+str(counter)+')Processing--')
    counter += 1
